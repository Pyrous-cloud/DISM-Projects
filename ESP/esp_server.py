...
def handler(con, client_addr):
    ...
    while True:
        try:
            # decode the bytes into printable str
            buf = con.recv(1000).decode()
            if len(buf) > 0:
                idle_count = 0
                ...
                try:
                    ...
                    log_file.close()
                    alive_clients_file = open(
                        'C:.\\esp_data\\alive_clients.json')
                    alive_clients = alive_clients_file.read()
                    alive_clients = json.loads(alive_clients)
                    alive_clients_file.close()
                except:
                    print(
                        'The file cannot be found or there is an error with the file...')
                    os._exit(1)
                        ...
                        if buf_list[1] not in alive_clients:
                            if buf_list[2] == users[buf_list[1]]['passwd']:
                                ...
                                try:
                                    ...
                                    alive_clients_file = open(
                                        'C:.\\esp_data\\alive_clients.json', 'w')
                                    alive_clients_file.write(
                                        json.dumps(alive_clients, indent=4))
                                    alive_clients_file.close()
                                except:
                                    print(
                                        'The file cannot be found or there is an error with the file...')
                ...
                # Client exits the program, server writes new data into the relevant files
                    ...
                    try:
                        ...
                        print(
                            'The file cannot be found or there is an error with the file...a')
                        os._exit(1)
            else:
                break
        # Code for timeout
            ...
            if str(inst) == 'timed out':
                ...
                print('handler {} time out: idle count is {}'.format(
                    str(client_addr), idle_count))
